...
import com.zhejiangshegndian.csw.adapter.AttentionAdapter;
...
public class AttentionActivity extends MyBaseActivity implements SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener {

    @InjectView(R.id.layout_back)
    LinearLayout layoutBack;
    @InjectView(R.id.list_attention)
    ...
    private void inits() {
        list = new ArrayList<>();
        adapter = new AttentionAdapter(this, list);
    }

    private void initListView() {
        listAttention.setOnItemClickListener(this);
        listAttention.setAdapter(adapter);
    }
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        if(list.size() > 0){
            startActivity(new Intent(AttentionActivity.this, PersonActivity.class)
    ...
    private void getPeople() {
        ...
        new Xutil().post(Constants.CODE_805091, object.toString(), new Xutil.XUtils3CallBackPost() {
            ...
            public void onTip(String tip) {
                Toast.makeText(AttentionActivity.this, tip, Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onError(String error, boolean isOnCallback) {
                Toast.makeText(AttentionActivity.this, "无法连接服务器，请检查网络", Toast.LENGTH_SHORT).show();
            }
        });
